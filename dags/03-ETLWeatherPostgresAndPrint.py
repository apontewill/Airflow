...
@dag(
    dag_id="ETLWeatherPostgresAndPrint",
    schedule=None,
    start_date=pendulum.datetime(2021, 1, 1, tz="UTC"),
    catchup=False,
    tags=["LearnDataEngineering"],
)
def etl_weather_postgres_and_print():
    """Fetch weather and fan out to PostgreSQL and task-log outputs."""
    ...
    @task
    def print_weather(weather_records: list[WeatherRecord]) -> None:
        print(weather_records)

    weather_data = extract()
    weather_summary = transform(weather_data)
    load(weather_summary)
    print_weather(weather_summary)
